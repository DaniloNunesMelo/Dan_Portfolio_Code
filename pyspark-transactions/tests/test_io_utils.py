...
These tests target the previously uncovered lines 19-20, 25-26, 35-54
...
class TestReadCsv:
    def test_reads_valid_csv(self, spark, tmp_path):
        ...
        assert rows[1]["AGE"] == "25"  # inferSchema=False → strings

    def test_reads_with_infer_schema(self, spark, tmp_path):
        csv_file = tmp_path / "test.csv"
        csv_file.write_text("NAME,AGE\nAlice,30\nBob,25\n", encoding="utf-8")

        df = read_csv(spark, str(csv_file), infer_schema=True)
        rows = df.collect()
        assert rows[0]["AGE"] == 30  # inferred as int

    def test_reads_with_custom_delimiter(self, spark, tmp_path):
        csv_file = tmp_path / "test.tsv"
        csv_file.write_text("NAME\tAGE\nAlice\t30\n", encoding="utf-8")

        df = read_csv(spark, str(csv_file), delimiter="\t")
        rows = df.collect()
        assert len(rows) == 1
        assert rows[0]["NAME"] == "Alice"
    ...
    def test_file_not_found_raises(self, spark, tmp_path):
        with pytest.raises(FileNotFoundError, match="Input file not found"):
            read_csv(spark, str(tmp_path / "nonexistent.csv"))

    def test_empty_csv_returns_empty_df(self, spark, tmp_path):
...
class TestWriteCsv:
    def _make_df(self, spark: SparkSession) -> DataFrame:
        ...
        return spark.createDataFrame([("x", "1"), ("y", "2")], schema)
    ...
    def test_writes_single_csv_file(self, spark, tmp_path):
        ...
        assert out_path.is_file()  # single file, not a directory
        content = out_path.read_text(encoding="utf-8")
        assert "A,B" in content  # header present
    ...
    def test_temp_dir_is_cleaned_up(self, spark, tmp_path):
        ...
        # The .tmp_output directory should be removed
    ...
    def test_writes_with_custom_delimiter(self, spark, tmp_path):
    ...
    def test_output_parent_dir_can_exist(self, spark, tmp_path):
        """Ensure writing works when parent dir already exists."""
        sub = tmp_path / "subdir"
        sub.mkdir()
    ...
    def test_multi_partition_coalesced_to_one(self, spark, tmp_path):
        """Even a repartitioned DF produces a single output file."""
        out_path = tmp_path / "output.csv"
